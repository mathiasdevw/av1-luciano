...
export async function GET(){
 if(!databaseConfigured()&&process.env.VERCEL_ENV==='production')return Response.json({error:'Serviço temporariamente indisponível.'},{status:503,headers:{'Cache-Control':'no-store'}});
 ...
 }catch{console.error('ranking_request_failed');return Response.json({error:'Não foi possível carregar o ranking. Tente novamente.'},{status:503,headers:{'Cache-Control':'no-store'}});}
}
